...
This form of scaling of the data transforms the data so it falls between a known lower and upper bound, often 0 and 1. Suppose that $\text{min}(X) \le X \le \text{max}(X)$  and we want to create
a variable $z_{\text{min}} \le Z \le z_{\text{max}}$ from $X$. $Z$ can be computed by scaling and shifting a standardized form of $X$:
$$
\begin{align*}
