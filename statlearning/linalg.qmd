...
We also say that $Y$ is **normally** distributed with mean $\mu$ and variance $\sigma^{2}$ but prefer the name Gaussian over Normal for the reasons given in @sec-prob-cont-dist. The shorthand expressions $Y \sim G(\mu,\sigma^{2})$ or $Y \sim N(\mu,\sigma^{2})$ are common.
...
Suppose that we want to find an estimator for $\bbeta$ in the linear model $\bY = \bX\bbeta + \bepsilon$. If the model errors follow a Gaussian distribution with mean $\bzero$ and variance $\bV$, then $\bY$ follows a Gaussian distribution because it is a linear function of $\bepsilon$. The probability density function of $\bY$ is
...
This joint distribution of the data can be used to derive the maximum likelihood estimator (MLE) of $\bbeta$. Maximum likelihood estimation considers this as a function of $\bbeta$ rather than a function of $\bY.$ Maximizing this likelihood function $\mathcal{l(}\bbeta;\bY)$ is equivalent to maximizing its logarithm and working on the log scale is much simpler. The log-likelihood fu1nction for this problem is given by
...
Finding the maximum of this function with respect to $\bbeta$ is equivalent to minimizing the quadratic form $\left( \bY - \bX\bbeta \right)^\prime\bV^{-1}\left( \bY - \bX\bbeta \right)$ with respect to $\bbeta$. Applying the results about matrix differentiation from above we obtain
...
If $\bX$ is of full column rank, then $\bX^\prime\bV^{- 1}\bX$ is non-singular and its inverse exists. Pre-multiplying both sides of the equation with that inverse yields the solution
...
We can write the error distribution in this case as $\bepsilon \sim G\left(\bzero,\sigma^{2}\bI \right)$ and the model for $\bY$ as $\bY \sim G\left( \bX\bbeta,\sigma^{2}\bI \right)$. This is known as the *iid* case, the errors are independent and identically distributed. You will encounter the *iid* assumption a lot in statistical modeling and in machine learning, because it allows you to write multivariate joint distributions as products of univariate distributions and this greatly simplifies matters.

Under the *iid* assumption for the Gaussian linear model we can substitute $\sigma^{2}\bI$ for $\bV$ in the formula for $\widehat{\bbeta}$ and obtain

$${\widehat{\bbeta}=\left( \bX^{\prime}\bX \right)}^{- 1}\bX^\prime\bY$$

the ordinary least squares estimator. Notice that $\sigma^{2}$ cancels out of the formula; the value of the OLS estimator does not depend on the intrinsic variability of the data. However, the variability of the OLS estimator does depend on $\sigma^{2}$ (and on $\bX$).
...
Because of this remarkable result, leave-one-out statistics can be calculated easily---without retraining any models---based on the fit to the full training data alone. Note that the quantity in the denominator of the right-hand side is the diagonal value of $\bI - \bH$, where $\bH$ is the hat matrix. If $h_{ii}$ denotes the diagonal values of $\bH$, we can write the update formula as
