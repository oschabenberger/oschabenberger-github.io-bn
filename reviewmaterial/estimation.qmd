...
which is identical to the rank of $\bXpX$. With these quantities available, the 
variance-covariance matrix of $\widehat{\bbeta}$, 
$$
...
and we can write a simple `R` function to compute the derivative matrix $\bF$ 
...
step in the updates since there is no guarantee that SSE is lower at iterate $t+1$ 
...
The following code computes the log-likelihood function $\mathcal{l}(\lambda; \by)$ for 
...
because it is a linear function of $\bepsilon$ (see @sec-multi-gauss). The 
